let lastProfitLossResult = null;
let lastDividendResult = null;
let lastRrrResult = null;
...
function showSaveModal(resultModalId) {
    ...
    checkAuthStatus().then(isAuthenticated => {
        if (!isAuthenticated) {
            showAlert('error', 'Authentication Required', 'Please sign in to save your calculations');
        ...
        // Если авторизован, показываем модальное окно сохранения
...
async function checkAuthStatus() {
    try {
        ...
        return response.ok; // Если статус 200, пользователь авторизован
    } catch (error) {
        return false;
    }
}
...
function saveTrade(resultModalId) {
    ...
    checkAuthStatus().then(isAuthenticated => {
        ...
        // Собираем данные из формы и результатов
        const calculationData = {
            title: tradeTitle,
            date: tradeDate || null
        };

        // Добавляем данные в зависимости от типа расчета
        if (resultModalId === 'profitloss-modal') {
            if (!lastProfitLossResult) {
                showAlert('error', 'Error!', 'No calculation results found. Please calculate first.');
                return;
            }

            const assetType = document.getElementById('pl-type').value;
            calculationData.asset_type = assetType;
            ...
            if (assetType === 'forex') {
                calculationData.open_price = getFieldValue('pl-forex-open-price');
                calculationData.close_price = getFieldValue('pl-forex-close-price');
                calculationData.amount = getFieldValue('pl-forex-lots');
                calculationData.volume = parseInt(document.getElementById('pl-forex-volume')?.value) || 0;
                calculationData.leverage = getFieldValue('pl-forex-lev');
            } else {
                calculationData.open_price = getFieldValue(`pl-${assetType}-open-price`);
                calculationData.close_price = getFieldValue(`pl-${assetType}-close-price`);
                calculationData.amount = getFieldValue(`pl-${assetType}-amount`);

                const leverageField = document.getElementById(`pl-${assetType}-leverage`);
                calculationData.leverage = leverageField?.value ? parseFloat(leverageField.value) : null;
                calculationData.volume = null;
            }

            // Используем рассчитанные результаты
            calculationData.position_size = lastProfitLossResult.position_size;
            calculationData.profit_loss = lastProfitLossResult.profit_loss;
            calculationData.profit_loss_yield = lastProfitLossResult.profit_loss_yield;
            calculationData.margin = lastProfitLossResult.margin || null;

        } else if (resultModalId === 'div-modal') {
            // ... существующий код для dividend ...
            if (!lastDividendResult) {
                showAlert('error', 'Error!', 'No calculation results found. Please calculate first.');
                return;
            }

            // Безопасное получение значений для dividend
            const getDivFieldValue = (id) => {
                const element = document.getElementById(id);
                return element ? parseFloat(element.value) || 0 : 0;
            };

            calculationData.price_of_1_share = getDivFieldValue('div-priceshare');
            calculationData.from_currency = document.getElementById('div-fromcurrency')?.value || 'USD';
            calculationData.number_of_shares = getDivFieldValue('div-numbershares');
            calculationData.div_per_1_share = getDivFieldValue('div-divshare');
            calculationData.pay_period = document.getElementById('div-payperiod')?.value || 'year';
            calculationData.own_period = getDivFieldValue('div-ownperiod');

            const taxRateInput = document.getElementById('div-taxrate');
            calculationData.tax_rate = taxRateInput?.value ? parseFloat(taxRateInput.value) : null;

            const divGrowthInput = document.getElementById('div-divgrowth');
            calculationData.div_growth = divGrowthInput?.value ? parseFloat(divGrowthInput.value) : null;

            // Используем рассчитанные результаты
            calculationData.total_div = lastDividendResult.total_div;
            calculationData.div_yield = lastDividendResult.div_yield;
            calculationData.total_div_yield = lastDividendResult.total_div_yield;
            calculationData.invest = lastDividendResult.invest || (calculationData.price_of_1_share * calculationData.number_of_shares);
            calculationData.ann_div_yield = lastDividendResult.ann_div_yield || null;
            calculationData.total_period_div_yield = lastDividendResult.total_period_div_yield || null;
            calculationData.total_return = lastDividendResult.total_return || lastDividendResult.total_div_yield;
            calculationData.ave_ann_ret = lastDividendResult.ave_ann_ret;

        } else if (resultModalId === 'rrr-modal') {
            // ... существующий код для RRR ...
            if (!lastRrrResult) {
                showAlert('error', 'Error!', 'No calculation results found. Please calculate first.');
                return;
            }

            // Безопасное получение значений для RRR
            const getRrrFieldValue = (id) => {
                const element = document.getElementById(id);
                return element ? parseFloat(element.value) || 0 : 0;
            };

            calculationData.open_price = getRrrFieldValue('open-price');
            calculationData.take_profit = getRrrFieldValue('take-profit');
            calculationData.stop_loss = getRrrFieldValue('stop-loss');
            calculationData.balance = getRrrFieldValue('balance');
            calculationData.risk_per_trade = getRrrFieldValue('risk-per-trade');

            // Используем рассчитанные результаты
            calculationData.position_size = lastRrrResult.position_size;
            calculationData.position_cost = lastRrrResult.position_cost;
            calculationData.rrr = lastRrrResult.rrr;
            calculationData.profit_per_share = lastRrrResult.profit_per_share;
            calculationData.risk_per_share = lastRrrResult.risk_per_share;
            calculationData.total_profit = lastRrrResult.total_profit;
            calculationData.total_risk = lastRrrResult.total_risk;
            calculationData.balance_after_profit = lastRrrResult.balance_after_profit;
            calculationData.balance_after_loss = lastRrrResult.balance_after_loss;
        }

        // Определяем endpoint
        let saveEndpoint = '';
        if (resultModalId === 'profitloss-modal') {
            saveEndpoint = '/save_pl';
        } else if (resultModalId === 'div-modal') {
            saveEndpoint = '/save_div';
        } else if (resultModalId === 'rrr-modal') {
            saveEndpoint = '/save_rrr';
        }
        ...
        .then(data => {
            if (data.success) {
                ...
                if (resultModalId === 'profitloss-modal') lastProfitLossResult = null;
                if (resultModalId === 'div-modal') lastDividendResult = null;
                if (resultModalId === 'rrr-modal') lastRrrResult = null;
        ...
        .catch(error => {
            if (error.message === 'Authentication required') {
                ...
                showAlert('error', 'Error!', 'Failed to save: ' + error.message);
            }
            document.getElementById('save-modal').classList.remove('active');
        });
    });
