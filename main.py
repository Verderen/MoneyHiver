...
from models import User, Asset, SavedCalculations
        ...
        if prices:
            for email, subscription in list(subscribers.items()):
                ...
                if current_price and current_price >= target_price:
                    # Используем Resend вместо SMTP
                    success = await EmailService.send_price_alert(
                        email, asset, current_price, target_price
                    )
                    if success:
                        del subscribers[email]
                        print(f"✅ Price alert sent and subscription removed for {email}")
...
async def message(
    ...
    try:
        # Используем Resend для отправки
...
async def get_saved_pl(request: Request):
    """Получить список сохраненных Profit/Loss расчетов"""
    user = get_current_user(request)
    if not user:
...
async def get_saved_div(request: Request):
    """Получить список сохраненных Dividend расчетов"""
    user = get_current_user(request)
    if not user:
        return JSONResponse({"error": "Authentication required"}, status_code=401)
...
async def get_saved_rrr(request: Request):
    """Получить список сохраненных RRR расчетов"""
    user = get_current_user(request)
    if not user:
        return JSONResponse({"error": "Authentication required"}, status_code=401)
...
async def get_pl_details(request: Request, id: int):
    """Получить детали Profit/Loss расчета"""
    user = get_current_user(request)
    if not user:
        return JSONResponse({"error": "Authentication required"}, status_code=401)
...
async def get_div_details(request: Request, id: int):
    """Получить детали Dividend расчета"""
    user = get_current_user(request)
    if not user:
        return JSONResponse({"error": "Authentication required"}, status_code=401)
